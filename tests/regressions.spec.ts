...
test('primeiro acesso e recarga preservam os dados, inclusive por HTTP de rede', async ({ page }) => {
  await page.goto('/');
  await expect(page.getByRole('button', { name: 'Catálogo + lista da planilha' })).toBeVisible();
  ...
  await page.getByRole('button', { name: 'Catálogo + lista da planilha' }).click();
  await expect(page.locator('.shopping-row.in-cart')).toHaveCount(17);
  const saved = await page.evaluate(() => localStorage.getItem('lista-compras-inteligente'));
  await page.reload();
  await expect(page.locator('.shopping-row.in-cart')).toHaveCount(17);
  expect(await page.evaluate(() => localStorage.getItem('lista-compras-inteligente'))).toBe(saved);
  await expect(page.getByRole('alert')).toHaveCount(0);
});

test('revisão espelha a lista, preserva carrinho e remove inclusive a última seleção', async ({ page }) => {
...
test('aparência clara, escura e automática acompanha sistema e persiste sem alterar compras', async ({ page }, testInfo) => {
...
test('histórico existente é agrupado sem regravar ou alterar dados', async ({ page }, testInfo) => {
  await page.goto('/'); await page.getByRole('button', { name: 'Catálogo + lista da planilha' }).click();
  ...
  const raw = await page.evaluate(() => localStorage.getItem('lista-compras-inteligente'));
  ...
  expect(await page.evaluate(() => localStorage.getItem('lista-compras-inteligente'))).toBe(raw);
